     ...
     int crossmeth, double split_alpha, double cv_alpha, int bucketnum, int bucketMax, double gamma)
{
    ...
    //int neighbor; // nearest neighbor number, just needed for matching!
    alphasave = ct.alpha;
    ...
    for (xgroup = 0; xgroup < n_xval; xgroup++) {
        ...
        for(i = k; i < ct.n; i++) {
            ...
            if (crossmeth == 1) {
                // tot-H
                totH_rundown(xtree, j, cp, xpred, xtemp, k, cv_alpha, xtrain_to_est_ratio, ct.propensity);
            } else if (crossmeth == 2) {
                // tot-A
                //neighbor = findNeighbor(j, k);
                //matching_rundown(xtree, j, neighbor, cp, xpred, xpred2, xtemp);
                totA_rundown(xtree, j, cp, xpred, xtemp, k, cv_alpha);
            } else if (crossmeth == 3) {
                //fit-honest: alpha need to change to cv.alpha, discriminate with split.alpha
                fitH_rundown(xtree, j, cp, xpred, xtemp, k, cv_alpha, xtrain_to_est_ratio, ct.propensity);
            } else if (crossmeth == 4) {
                // fit-adaptive:
                fitA_rundown(xtree, j, cp, xpred, xtemp, k, cv_alpha);
