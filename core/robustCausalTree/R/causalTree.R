...
causalTree <- function(formula, data, weights, treatment, subset,
  ...
  # model.response<-model.weights <-model.offset <- .getXlevels <- rpart.control <-
	...
	split.Rule.int <- pmatch(split.Rule, c("MSD", "CT", "MAD", "tstats", "MSDD", "CTD",
	                                       "MADD", "tstatsD", "LMS", "LMSD","policy","policyD"))
	# print(split.Rule.int)
	# print(split.Rule)
	if (is.na(split.Rule.int)) stop("Invalid splitting rule.")
	split.Rule <- c("MSD", "CT", "MAD", "tstats", "MSDD", "CTD", "MADD",
	                "tstatsD", "LMS", "LMSD","policy","policyD")[split.Rule.int]
	...
	# check propensity score:
	# if (split.Rule %in% c("TOT", "TOTD", "MAD", "MADD")) {
	...
	if (cv.option == "CT" || cv.option == "MAD" || cv.option == "MSD" || cv.option == "LMS" || cv.option == "policy") {
		if (cv.Honest) {
			# cv.Honest = T
	...
	cv.option.num <- pmatch(cv.option, c("MSDH", "MSDA", "MADH", "MADA", "CTH", "CTA", "LMSH", "LMSA","policyH","policyA", "none"))
...
####
		ctfit <- .Call("causalTree",PACKAGE="robustCausalTree",
					   ...
					   split_Rule = as.integer(split.Rule.int), # MSD, ct, fit, tstats, MSDD, ctD, fitD, tstatsD
