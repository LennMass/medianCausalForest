...
BLB_scaling_vector <- c(TRUE, FALSE)

path_out <- "simulation/raw_results_BLB/"
...
for (s_i in seq_len(nrow(scenarios))) {
  ...
  for (seed in setseed_vec) {
    ...
    for (sim_i in seq_len(sims)) {
      ...
      forest_rows <- list()
      for (BLB_scaling in BLB_scaling_vector){
        ...
        print(paste("Finished BLB scaling", BLB_scaling))
        
      } # end blb scaling
      
      
      # ---- 5. assemble this replication ----
      one_sim <- rbind(do.call(rbind, forest_rows))
      one_sim$sim <- sim_i
      rows[[sim_i]] <- one_sim
